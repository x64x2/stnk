...
std::endpoint::~endpoint()
{
    close();
}


void std::endpoint::close()
{
    for (;;)
    {
        if (::close(fd) == 0)
...
std::endpoint::endpoint(int a_fd):fd(a_fd),ready_to_die(false)
{
    set_peer_name();
}

std::endpoint::endpoint(int a_fd, const void *sadr, int sadr_len):fd(a_fd),ready_to_die(false)
{
